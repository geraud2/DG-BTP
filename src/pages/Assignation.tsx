...
import { User, Building2, Calendar, Flag, Plus, Trash2, CheckCircle2, Clock, AlertCircle, Search, X, Briefcase, ChevronDown, Send } from 'lucide-react';
...
export default function AssignationTaches() {
  ...
  const getAvatarColor = (name: string) => {
    const colors = ['bg-amber-100 text-amber-700', 'bg-sky-100 text-sky-700', 'bg-emerald-100 text-emerald-700', 'bg-violet-100 text-violet-700', 'bg-rose-100 text-rose-700', 'bg-teal-100 text-teal-700'];
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div><h1 className="text-xl sm:text-2xl font-bold text-gray-900">Assignation des tâches</h1><p className="text-gray-500 text-xs sm:text-sm mt-0.5">Assigner des tâches au personnel</p></div>
        <div className="flex items-center gap-2"><Briefcase size={18} className="text-amber-600" /><span className="text-xs sm:text-sm font-semibold text-gray-700">{taches.length} tâches</span></div>
      </div>

      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="bg-white border border-sky-200 rounded-xl p-3 text-center"><Clock size={18} className="text-sky-600 mx-auto mb-1" /><p className="text-xl sm:text-2xl font-bold text-sky-600">{tachesActives.length}</p><p className="text-[10px] sm:text-xs font-medium text-sky-600">En cours</p></div>
        <div className="bg-white border border-red-200 rounded-xl p-3 text-center"><AlertCircle size={18} className="text-red-600 mx-auto mb-1" /><p className="text-xl sm:text-2xl font-bold text-red-600">{tachesEnRetard.length}</p><p className="text-[10px] sm:text-xs font-medium text-red-600">En retard</p></div>
        <div className="bg-white border border-emerald-200 rounded-xl p-3 text-center"><CheckCircle2 size={18} className="text-emerald-600 mx-auto mb-1" /><p className="text-xl sm:text-2xl font-bold text-emerald-600">{tachesTerminees.length}</p><p className="text-[10px] sm:text-xs font-medium text-emerald-600">Terminées</p></div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 sm:gap-6">
        <Card title="Nouvelle assignation" icon={Plus}>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block"><Building2 size={12} className="inline mr-1" /> Département</label>
              <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition">{departments.map((d) => (<option key={d} value={d}>{d}</option>))}</select>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block"><User size={12} className="inline mr-1" /> Employé</label>
              {selectedEmployeeData ? (
                <div className="flex items-center justify-between p-3 bg-amber-50 border border-amber-200 rounded-xl">
                  <div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${getAvatarColor(selectedEmployeeData.name)}`}>{getInitials(selectedEmployeeData.name)}</div><div><p className="text-sm font-semibold text-gray-900">{selectedEmployeeData.name}</p><p className="text-xs text-gray-500">{selectedEmployeeData.role}</p></div></div>
                  <button type="button" onClick={() => { setSelectedEmployee(''); setSearchEmployee(''); }} className="p-1 hover:bg-red-50 rounded-lg"><X size={16} className="text-gray-400 hover:text-red-500" /></button>
                </div>
              ) : (
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Titre de la tâche *</label>
              <input type="text" value={newTache.title} onChange={(e) => setNewTache((prev) => ({ ...prev, title: e.target.value }))} placeholder="Ex: Superviser migration CRM" className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Description</label>
              <textarea value={newTache.description} onChange={(e) => setNewTache((prev) => ({ ...prev, description: e.target.value }))} placeholder="Détails de la tâche..." rows={3} className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition resize-none" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block"><Calendar size={12} className="inline mr-1" /> Date début</label><input type="date" value={newTache.dateDebut} onChange={(e) => setNewTache((prev) => ({ ...prev, dateDebut: e.target.value }))} className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition" /></div>
              <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block"><Calendar size={12} className="inline mr-1" /> Échéance *</label><input type="date" value={newTache.dateEcheance} onChange={(e) => setNewTache((prev) => ({ ...prev, dateEcheance: e.target.value }))} className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition" /></div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block"><Flag size={12} className="inline mr-1" /> Priorité</label>
              <div className="flex gap-2">{(['haute', 'moyenne', 'basse'] as const).map((p) => (<button key={p} type="button" onClick={() => setNewTache((prev) => ({ ...prev, priority: p }))} className={`flex-1 py-2.5 text-xs sm:text-sm font-semibold rounded-xl transition-all ${newTache.priority === p ? `${priorityConfig[p].color} border-2 font-bold` : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-100'}`}>{priorityConfig[p].icon} {priorityConfig[p].label}</button>))}</div>
            </div>
